...
%       date: 06/22/15
%    purpose: Tactile Amplitude Discrimination With Post-Mask
%
...
%[ampStim] = makeAmpTimeStim(offset, deltaN, eventLength, freq, Lbase, ampMid, ampUp)

%1: proximal stimulator

function myscreen = somAmpTimingCrossed()
...
%variable stimulus parameter - gap between two middle stimulations
task{1}.parameter.deltaN = [-1*flipdim(deltas,2) deltas];
...
if task.thistrial.thisseg == 1
  ...
  deltaN = task.thistrial.deltaN;

  eventLength = task.thistrial.eventLength;
  freq = task.thistrial.freq;

[trialStim] = makeAmpTimeStim(offset, deltaN, eventLength, freq, Lbase, ampMid, ampUp);  
